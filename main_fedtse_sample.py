...
import torch
from src.data_process_sample import load_data_pneuma, get_time_series_pneuma, load_data_sumo, get_time_series_sumo
...
def get_local_model(num, MULTIGRAPH, learning_rate, ADJPATH, o, mode):
    ...
    for i in range(GRAPHNUMBER):
        ...
        Lk_new[i] = Lk
    # print(Lk_new.shape)
    model = STGCN(ks, kt, bs, T, n, Lk_new, p, learning_rate, o, mode).to(device)
    return model
...
def run_experiment(train_data, val_data, test_data, batch_size, learning_rate, epoch, adj_matrix):
    ...
    party_a_local_model = get_local_model(1, False, learning_rate, adj_matrix, 2, mode = 'vfl')
    party_b_local_model = get_local_model(2, False, learning_rate, adj_matrix, 2, mode = 'vfl')
    party_a_global_model = conv(learning_rate).to(device)
...
if __name__ == '__main__':
    for xxx in [1, 2, 3, 4, 5, 6]:
        ...
        KEYWORD = 'fedtse' + '_' + f'MP_{xxx}' + f'_{LEARN}' + '_' + datetime.now().strftime("%y%m%d%H%M")
        PATH = '../save_sumo_fedtse_sample/' + KEYWORD
        ...
        if train_mode == 'simulation':
            # load data
            X_MA_train, X_MP_train, Y_train, X_MA_val, X_MP_val, Y_val, X_MA_test, X_MP_test, Y_test, scaler = load_data_sumo(MA_obs, PRMP, cell_rep, xxx)
