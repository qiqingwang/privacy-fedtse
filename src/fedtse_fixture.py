    ...
    def fit(self, train_data, val_data, test_data, epochs=50, batch_size=-1, PATH=None, scaler=None):
        ...
        for ep in range(epochs):
            ...
            for batch_idx in range(n_batches):
                ...
                train_loss = self.federated_learning.fit(Xa_batch, Y_batch,
            ...
            # MAPE
            # 对于第一个特征
            non_zero_mask_q = y_test[:,0,:,:] > 10
            mape_values_q = np.abs((y_test[:,0,:,:] - test_y_preds[:,0,:,:]) / y_test[:,0,:,:])
            test_mape_q = np.mean(mape_values_q[non_zero_mask_q]) * 100
           # 对于第二个特征
            non_zero_mask_k = y_test[:,1,:,:] > 10
            mape_values_k = np.abs((y_test[:,1,:,:] - test_y_preds[:,1,:,:]) / y_test[:,1,:,:])
            test_mape_k = np.mean(mape_values_k[non_zero_mask_k]) * 100
