...
class VFLHostMA(object):
    ...
    def _compute_common_gradient_and_loss(self, y):
        ...
        # lane_4 = [3,4,5,6]
        # lane_5 = [0,1,2]
        # U[:,:,:,lane_5] = U[:,:,:,lane_5] / 2.236068
        # y[:,:,:,lane_5] = y[:,:,:,lane_5] / 2.236068
        # U[:,:,:,lane_4] = U[:,:,:,lane_4] / 2
        # y[:,:,:,lane_4] = y[:,:,:,lane_4] / 2
        # U[:,:,:,lane_2] = U[:,:,:,lane_2] / 1.4142136
        # y[:,:,:,lane_2] = y[:,:,:,lane_2] / 1.4142136
    ...
    def send_gradients(self):
        # divided_grads = []
        # start_idx = 0
        # for comp_tensor in self.parties_grad_component_list:
        #     end_idx = start_idx + comp_tensor.shape[1]  # 假设你是沿着dim=1进行拼接的
        #     cur_grad = self.back_grad_wrt_Z[:, start_idx:end_idx]
        #     divided_grads.append(cur_grad)
        #     start_idx = end_idx
